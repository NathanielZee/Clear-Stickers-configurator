...
function priceClear(sizeMm: number, qty: number) {
  ...
  // Calculate savings based on 50-unit baseline for this size
  const base50Total = table["50"]
...
export default function StickerCalculator() {
  ...
  if (quantity > 0) {
    try {
      const sizeMm = width // Use width as the size dimension for clear stickers
      pricingResult = priceClear(sizeMm, quantity)
